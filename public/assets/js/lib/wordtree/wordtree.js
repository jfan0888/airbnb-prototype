...
function wordtree() {
  var width,
      ...
      root,
      phraseLine = false,
      event = d3v3.dispatch("prefix");

  var scale = d3v3.scale.sqrt().range([0, 25]);

  var layout = d3v3.layout.partition()
        .sort(wordtreeSort)
  ...
  return d3v3.rebind(d3v3.rebind(wordtree, event, "on"), layout, "sort");
  ...
  function fullPrefix(d) {
    var s = [];
    do {
      (reverse ? s.push : s.unshift).call(s, d3v3.merge(d.tokens.map(function(d) { return [d.whitespace ? " " : "", d.token]; })));
    } while (d = d.parent);
    s[0] = s[0].slice(1);
    return d3v3.merge(s).join("");
  }
...
/*
     FILE ARCHIVED ON 22:36:23 Feb 15, 2014 AND RETRIEVED FROM THE
...
/*
playback timings (ms):
  LoadShardBlock: 89.432 (3)
  esindex: 0.005
  captures_list: 104.038
  CDXLines.iter: 8.26 (3)
  PetaboxLoader3.datanode: 147.741 (5)
  exclusion.robots: 0.129
  exclusion.robots.policy: 0.12
  RedisCDXSource: 3.695
  PetaboxLoader3.resolve: 84.565 (2)
  load_resource: 146.234
*/