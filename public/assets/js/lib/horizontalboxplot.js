...
// https://bl.ocks.org/asizer/11198007
d3v3.box = function() {
    var width = 1,
        height = 1,
        duration = 0,
    ...
    function box(g) {
        g.each(function(d, i) {
            ...
            var g = d3v3.select(this).attr('class', 'boxplot'),
            ...
            bg_gradient
                .append('stop')
                .attr('offset', '0%')
                .attr('stop-color', backgroundColor.start);

            bg_gradient
                .append('stop')
                .attr('offset', '100%')
                .attr('stop-color', backgroundColor.stop);

            var bg_rect = g.append('rect')
                .attr('fill', 'url(#bg_gradient)')
                .attr({
                    x: 0,
                    y: -10,
                    width: width,
...
function boxQuartiles(d) {
    return [
        d3v3.quantile(d, 0.25),
        d3v3.quantile(d, 0.5),
        d3v3.quantile(d, 0.75)
    ];
}