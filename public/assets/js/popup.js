...
var Popup = (function _popup() {
  ...
  function miniPopup(aData, bShowOnlyCount) {
    ...
    if (bShowOnlyCount) {
      ...
      headerText = [aData.length, sRecord].concat([
        'in',
        _.startCase(firstFeature.City) + ',',
        firstFeature.State,
      ]);
    }
    ...
    list.html(
      Mustache.render(profileScoreHTML, {
        profiles: aData,
      })
    );

    list.selectAll('li').on('click', function(d, i) {
  ...
  function profilePopup(oData) {
    ...
    //
    el.selectAll('.profilepanel__bookmark').on('click', function(d) {
      // get profile ID
      //
      var btn = jQuery(this);
    ...
    //
    el.selectAll('.profilepanel__back').on('click', function(d) {
      // Remove active class
      //
      jQuery('.popup--activeprofile').removeClass('popup--activeprofile');

      jQuery('.profilepanel--active').removeClass('profilepanel--active');
    });

    // Make a Profile Active
    //
    el.selectAll('.profilepanel--handle').on('click', function(d) {
      // If not already active class
      //
      var $panel = jQuery(this).closest('.profilepanel');
      if (!$panel.hasClass('profilepanel--active')) {
        jQuery('.popup--large').addClass('popup--activeprofile');

        $panel.addClass('profilepanel--active');
      }
